...
val parts : Int = 4
val scGap : Float = 0.018f / parts
val delay : Long = 20
val strokeFactor : Float = 90f
val backColor : Int = Color.parseColor("#BDBDBD")
val blockFactor : Float = 9.2f
val hFactor : Float = 2.3f
...
class RotatingPullerView(ctx : Context) : View(ctx) {
    ...
    data class Renderer(var view : RotatingPullerView) {

        private val rp : RotatingPuller = RotatingPuller(0)
        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            rp.draw(canvas, paint)
            animator.animate {
                rp.update {
                    animator.stop()
                }
